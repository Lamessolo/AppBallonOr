...
public interface JoueurRepository extends JpaRepository<Joueur,Long> {
	...
	@Query("SELECT j FROM Joueur j WHERE (:posteId IS NULL OR j.poste.id = :posteId) AND (:anneeRecompense IS NULL OR j.anneeRecompense = :anneeRecompense) AND (:classement IS NULL OR j.classement = :classement)")
		Page<Joueur> findJoueurByAnneeRecompenseAndPosteIdAndClassement(@Param("anneeRecompense")String anneeRecompense, @Param("posteId")Integer posteId,@Param("classement")Integer classement, Pageable pageable);
